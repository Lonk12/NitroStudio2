...
namespace InstallerFolderPermissions {
    ...
    public partial class Installer1 : Installer {
        ...
        public override void Install(IDictionary stateSaver) {
            ...
            // Create the rules
            FileSystemAccessRule writerule = new(sid, FileSystemRights.Write, AccessControlType.Allow);

            if (!string.IsNullOrEmpty(folder) && Directory.Exists(folder)) {
                // Get your file's ACL
                DirectorySecurity fsecurity = FileSystemAclExtensions.GetAccessControl(folder);

                // Add the new rule to the ACL
                fsecurity.AddAccessRule(writerule);

                // Set the ACL back to the file
                FileSystemAclExtensions.SetAccessControl(folder, fsecurity);
            }
